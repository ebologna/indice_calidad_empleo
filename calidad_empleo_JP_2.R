...
indice = datos2[,c("seguridad","horas_trabajadas","ingreso_hs","ocupaciones","tasa_actividad",
                   "tasa_desempleo","prop_cuentapropistas","percentile_90")]
...
datos3 = datos2[,c("seguridad","horas_trabajadas","ingreso_hs","ocupaciones","tasa_actividad",
                   "tasa_desempleo","prop_cuentapropistas","percentile_90","sexo","año")]
summary(datos3)
...
tabla_eb = datos3 %>% 
  group_by(año, sexo) %>% 
  summarise(indice = mean(indice_acp, na.rm = T),
            segur = mean(seguridad), horas_trab=mean(horas_trabajadas),
            inghora=mean(ingreso_hora),
            ocups = mean(ocupaciones), activi=mean(tasa_actividad),
            desempl=mean(tasa_desempleo),
            cuantaprop=mean(prop_cuentapropistas), perc_90=mean(percentile_90))
