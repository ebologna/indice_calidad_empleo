...
library(BBmisc)
# lectura bases
eph_1_05<-read.dbf("bases_EPH/Ind_t105.DBF") # ver qué pasa con IPCF y la coma decimal en los dbf
...
# Tasa de actividad para personas de 25-64
# de estado, ocupados+desocupados/total
grupo_25_64<-subset(base, base$CH06>24 & base$CH06<65)
grupo_25_64$ESTADO[grupo_25_64$ESTADO==0]<-NA

tasa_actividad <- (((addmargins(wtd.table(grupo_25_64$ESTADO, weights = grupo_25_64$PONDERA)))[1]+
                      (addmargins(wtd.table(grupo_25_64$ESTADO, weights = grupo_25_64$PONDERA)))[2])/
                     (addmargins(wtd.table(grupo_25_64$ESTADO, weights = grupo_25_64$PONDERA)))[4])
tasa_actividad<-as.numeric(tasa_actividad)

actividad_sexo<-grupo_25_64 %>%
  group_by(CH04) %>% 
...
grupo_25_64$active<-0
grupo_25_64$active[grupo_25_64$ESTADO==1 | grupo_25_64$ESTADO==2]<-1
t.test(grupo_25_64$active~grupo_25_64$CH04)


actividad_region<-grupo_25_64 %>%
  group_by(REGION) %>% 
  summarise(valor=(((addmargins(wtd.table(ESTADO, weights = PONDERA)))[1]+
                      (addmargins(wtd.table(ESTADO, weights = PONDERA)))[2])/
                     (addmargins(wtd.table(ESTADO, weights = PONDERA)))[4]))
...
u <- aov(grupo_25_64$active~grupo_25_64$REGION)
w<-TukeyHSD(x=u, 'grupo_25_64$REGION', conf.level=0.95)
attributes(w)

actividad_edad<-grupo_25_64 %>%
...
u <- aov(grupo_25_64$active~grupo_25_64$grupos_edades)
TukeyHSD(x=u, 'grupo_25_64$grupos_edades', conf.level=0.95)

actividad_educacion<-grupo_25_64 %>%
  group_by(NIVEL_ED) %>% 
  summarise(valor=(((addmargins(wtd.table(ESTADO, weights = PONDERA)))[1]+
                      (addmargins(wtd.table(ESTADO, weights = PONDERA)))[2])/
                     (addmargins(wtd.table(ESTADO, weights = PONDERA)))[4]))
...
u <- aov(grupo_25_64$active~grupo_25_64$NIVEL_ED)
TukeyHSD(x=u, 'grupo_25_64$NIVEL_ED', conf.level=0.95)
...
valores_actividad<-c(tasa_actividad[[1]], actividad_sexo[[2]],
                     actividad_region[[2]], NA, actividad_edad[[2]],
...
valores_desig_ingreso_hora<-c(NA, gap_ingreso_hora_sexo[[2]],
                              gap_ingreso_hora_region[[2]],
                              gap_ingreso_hora_edad[[2]],
                              gap_ingreso_hora_educacion[[2]])


valores_diferencia_p90_p50<-c(p90_p50_total, p90_p50_sexos$diferencia, p90_p50_region$diferencia,
...
length(subset(base, base$CH04=="varones" & base$REGION == "Gran Buenos Aires" & 
         base$grupos_edades== "(0,24]" &
           base$NIVEL_ED == "primario incompleto o menos")$ESTADO)

# Anda, pero convendría simplificarlo, si no son 216 veces
...
names(u)<-c("sexo", "region", "edad", "educacion", "actividad", "desocupacion",
            "cuenta_propia", "seguridad", "media_ingresos", "media_ingresos_hora", "p90_p10")

# Modelos sin ponderadores
names(u)
m.1<-lm(actividad ~ sexo+region+edad+educacion, data = u)
m.2<-lm(desocupacion ~ sexo+region+edad+educacion, data = u)
m.3<-lm(cuenta_propia ~ sexo+region+edad+educacion, data = u)
m.4<-lm(seguridad ~ sexo+region+edad+educacion, data = u)
m.5<-lm(media_ingresos ~ sexo+region+edad+educacion, data = u)
m.6<-lm(media_ingresos_hora ~ sexo+region+edad+educacion, data = u)
m.7<-lm(p90_p10 ~ sexo+region+edad+educacion, data = u)

class(summary(m.1))
attributes(summary(m.1)[[4]])
coef_m.1<-as.data.frame(summary(m.1)[[4]])
class(rownames(coef_m.1))
nombres<-rownames(coef_m.1)<-as.factor(rownames(coef_m.1))
coef_m.1<-coef_m.1[,-c(2,3,4)]
s<-data.frame(nombres,coef_m.1)
names(s)<-c("froonteras", "actividad")
s
