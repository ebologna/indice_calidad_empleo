...
# Piketty dice que en Europa es 8 a 1, como esto
# hay que poder abrir esto para los grupos
# voy a reemplazar todos loa desigualdad ingreso hora por p90_p50


ingresos_bajos_mujeres<-subset(ingresos_bajos, ingresos_bajos$CH04==2)
ingresos_altos_mujeres<-subset(ingresos_altos, ingresos_altos$CH04==2)
p90_p50_mujeres<-wtd.mean(ingresos_altos_mujeres$IPCF, weights = ingresos_altos_mujeres$PONDERA)/
  wtd.mean(ingresos_bajos_mujeres$IPCF, weights = ingresos_bajos_mujeres$PONDERA)


ingresos_bajos_varones<-subset(ingresos_bajos, ingresos_bajos$CH04==1)
ingresos_altos_varones<-subset(ingresos_altos, ingresos_altos$CH04==1)
...
# sigue para cada categoría de región, de edad y de educación. Ufff, automatízalo!!!
...
compara_grupos_normalized$fronteras<-c("media nacional", rep("sexo",2), rep("region", 6), rep("edad",4), rep("educacion",7) )
compara_grupos_normalized$grupos <- factor(compara_grupos_normalized$grupos,
